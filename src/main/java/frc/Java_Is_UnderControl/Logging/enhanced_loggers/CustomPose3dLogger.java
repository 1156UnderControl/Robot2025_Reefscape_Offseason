package frc.Java_Is_UnderControl.Logging.enhanced_loggers;
...
public class CustomPose3dLogger extends Pose3dLogEntry {
  ...
  public CustomPose3dLogger(String name) {
    ...
    this.name = name;
    this.publisher =
        NetworkTableInstance.getDefault().getStructTopic(name, Pose3d.struct).publish();
    CustomPose3dLogger.isFmsMatch = DriverStation.getMatchNumber() > 0;
    this.loggedValue =
        new Pose3d(
            100, 100, 0, new Rotation3d()); // Set to something different than default for initial
    // // logging
    this.publisher =
        NetworkTableInstance.getDefault().getStructTopic(name, Pose3d.struct).publish();
